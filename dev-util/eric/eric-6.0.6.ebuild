...
src_prepare() {
	...
	sed -i -e 's/^Icon=eric$/&6/' eric/eric6.desktop || die
	sed -i -e 's/\([^[:alnum:]]\)eric\.png\([^[:alnum:]]\)/\1eric6.png\2/' \
		$(grep -lr 'eric\.png' .) || die
	mv eric/icons/default/eric{,6}.png || die
	mv eric/pixmaps/eric{,6}.png || die
...
src_install() {
	...
	doicon eric/icons/default/eric6.png
